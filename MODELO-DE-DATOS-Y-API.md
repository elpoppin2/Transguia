...
|---|---|
| `GET /api/dashboard?ventana=7d\|30d\|90d\|12m` | **Indicadores de gestión.** Todo medido sobre una ventana móvil (default `30d`; `12m` agrupa por mes). Devuelve: **7 KPI** con variación vs. el período previo — toneladas movidas, viajes, cumplimiento de programación (tickets ENTREGADO por fecha real de entrega vs. `viajes_programados` con fecha en la ventana, sin contar cancelados — no hay vínculo 1 a 1 entre un viaje programado y un ticket puntual, es plan vs. ejecución agregados), ciclo del traslado (h, con despacho/tránsito), tasa de anulación, utilización de flota, GRE aceptada por SUNAT (los últimos cuatro con semáforo `bien\|atencion\|critico`); y **series de apoyo** — toneladas por día, viajes por estado, toneladas por material, viajes por unidad (top 10), viajes por chofer (top 10), y corredores (toneladas por par origen→destino, para el heatmap). Para `admin_empresa` es su empresa; para `superadmin`, `?empresaId=<uuid>` una empresa o `todas` (acumulado). El `operador` no tiene acceso (403). |
