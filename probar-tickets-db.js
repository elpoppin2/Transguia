...
(async () => {
  ...
  const ticket = await esperarEmision(ticketService, {
    ...
    descripcionMercancia: 'Equipos de cómputo',
    pesoBrutoKg: 5400
  });
  ...
  try {
    await ticketService.crearTicket({
      ...
      choferId: chofer.id,
      origen: 'A', destino: 'B', descripcionMercancia: 'x', pesoBrutoKg: 1
    });
  ...
  const ticketRechazado = await esperarEmision(serviceRechaza, {
    ...
    unidadId: unidad.id,
    choferId: chofer.id,
    origen: 'Lima', destino: 'Piura', descripcionMercancia: 'Cajas', pesoBrutoKg: 3000
  });
